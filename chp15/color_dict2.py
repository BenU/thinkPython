...
if __name__ == '__main__':
  ...
  print colors_count
  for name, rgb in color_dict.iteritems():
    row = (count/tot_colors_root)
    column = (count%tot_colors_root)
    # bbox =[[lower left corner], [upper right corner]]
    ll_x = quadrant_x + (column * swath_width)
    ll_y = quadrant_y - (swath_height+(row * swath_height))
    ur_x = quadrant_x + (column * swath_width) + swath_width
    ur_y = quadrant_y - (row*swath_height)
    bbox = [[ll_x, ll_y], [ur_x, ur_y]]
    draw_rectangle(canvas, bbox, rgb)
    print count, row, column, rgb, name
    count += 1
  print colors_count
